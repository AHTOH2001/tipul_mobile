...
import { create_guardian, get_user_type } from '../../api/api';
...
class GuardianScreen extends Component {
    ...
    inputRelationshipUpdate = (val) => {
        this.inputValueUpdate(val, 'relationship')
    }
    ...
    onSubmitEditingFirstName = () => {
        if (this.state.last_name !== '' && this.state.relationship !== '' && this.state.phone !== '') {
            this.onSubmitEditing()
        }
    }

    onSubmitEditingLastName = () => {
        if (this.state.first_name !== '' && this.state.relationship !== '' && this.state.phone !== '') {
            this.onSubmitEditing()
        }
    }

    onSubmitEditingRelationsip = () => {
    ...
    onSubmitEditingPhone = () => {
        if (this.state.first_name !== '' && this.state.last_name !== '' && this.state.relationship !== '') {
            this.onSubmitEditing()
        }
    }
    ...
    validateRelationship = () => {
        if (this.state.relationship === '') {
            return { age_error: translate('Please, fill in your relationship', this.props.root.language) }
        }
        return false
    }
    ...
    onSubmitEditing = () => {
        ...
        var v3 = this.validateRelationship()
        ...
        if (!v1 && !v2 && !v3 && !v4) {
            ...
            create_guardian(this.state.first_name, this.state.last_name, this.state.relationship, this.state.phone).then(resp => {
                Alert.alert(
                    translate('Guardian created', this.props.root.language),
                )
                this.props.navigation.reset({
                    index: 0,
                    routes: [
    ...
    handleErrors(errors) {
        ...
        if ('relationship' in errors) {
            state.relationship_error = errors.relationship[0]
        }
        if ('phone' in errors) {
        ...
        return (
            <ScrollView style={styles.inputGroup}>
                ...
                <Input
                    ...
                    onChangeText={this.inputRelationshipUpdate}
                    leftIcon={{ type: 'font-awesome-5', name: 'user-tag' }}
                    onSubmitEditing={this.onSubmitEditingRelationsip}
                    label={translate('Relationship', this.props.root.language)}
                    errorMessage={this.state.relationship_error}
                />
                <Input
...
export default connect(mapStateToProps)(GuardianScreen)
